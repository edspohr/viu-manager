...
2. En el área "Solicitud / Correo", escribe:
   ```
...
### 3.2 Detección automática de cliente

Si la IA detecta el nombre de un cliente en el texto:

- **Cliente existente en catálogo:** Se selecciona automáticamente en el selector
- **Cliente nuevo:** Aparece el modal "Cliente detectado"
  - Prueba **"Crear nuevo cliente"** → el nombre viene prefillado → elige tipo → guarda → el cliente queda seleccionado
  - Prueba **"Vincular a cliente existente"** → elige uno del catálogo → queda seleccionado
  - Prueba **"Continuar sin cambios"** → cierra el modal sin hacer nada

### 3.3 Con archivo Excel o CSV
...
### 3.4 Con PDF o imagen

- Arrastra un PDF de cotización o una foto de un presupuesto
- La IA interpreta el contenido visual y extrae los ítems

### 3.5 Material desconocido

Si la IA detecta un material que no existe en el catálogo, aparece el modal "Material desconocido":
- **"Agregar al catálogo"** → ingresa nombre, elige tipo (Rígido/Flexible) → se crea con precio $0 para configurar después
- **"Mapear a material existente"** → elige el equivalente del catálogo
- **"Omitir"** → lo salta y queda para revisión manual

Si hay más de uno, el contador "+N más" indica cuántos quedan en la cola.

### 3.6 Editar la cotización antes de crear la orden

En el paso "Revisar" todo es editable:

| Campo | Cómo editar |
|---|---|
| Descripción | Clic directo en el texto |
| Ancho / Alto | Modifica el número — el precio se recalcula |
| Cantidad | Modifica el número — el subtotal se recalcula |
| Terminación | Clic en el selector → checkbox con todas las opciones |
| Precio unitario | Editable directamente — se marca en ámbar si fue modificado |
| Tooltip ⓘ | Pasa el cursor → muestra desglose de cálculo |
| Eliminar ítem | Botón papelera al final de la fila |
| Agregar ítem | "+ Agregar ítem" al pie de la tabla |

> **Validación importante:** Si cualquier ítem tiene ancho = 0 o alto = 0, el botón "Crear Orden" queda deshabilitado. Debes corregir las medidas antes de continuar.
